    ...
    <section className="py-16 lg:py-24 bg-[#FAFBFC]">
      <div className="max-w-7xl mx-auto px-4 lg:px-8">
        ...
        {caseStudies?.filter(study => study?.featured)?.map((study) => (
          <div
            key={study?.id}
            className="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 mb-8"
          >
              ...
              <div className="p-8 flex flex-col justify-center">
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  {study?.results?.map((result, index) => (
                    <div key={index} className="bg-[#F3F4F6] rounded-lg p-4 text-center">
                      <p className="text-xs text-[#4A5568] mb-1">{result?.metric}</p>
                      <p className="text-2xl font-bold text-[#1B365D]">{result?.value}</p>
                    </div>
                  ))}
                </div>
                ...
                <Link to="/korean-super-store-case-study">
        ...
        {/* Other Case Studies - Two Columns */}
            ...
            >
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-3 gap-3 mb-4">
                  {study?.results?.map((result, index) => (
                    <div key={index} className="bg-[#F3F4F6] rounded-lg p-3 text-center">
                      <p className="text-xs text-[#4A5568] mb-1">{result?.metric}</p>
                      <p className="text-lg font-bold text-[#1B365D]">{result?.value}</p>
                    </div>
                  ))}
                </div>
